...
HEADERS = "BatteryPercentage|BatteryVoltage|BatteryTemperature|ControllerTemperature|ChargingStatus|LoadVoltage|LoadCurrent|LoadPower|SolarVoltage|SolarCurrent|SolarPower".split("|")
	...
	while datetime.date.today() == curr_date:
		try:
			with open(fname, 'a', newline = "") as w:
				...
				#TODO add time
				csv_writer.writerow([controller.battery_percentage(),
				...
				'''
				csv.writer(w).writerow([str(randint(0,10)) for i in range(len(HEADERS))])
				#w.flush()
				#os.fsync(w.fileno())
